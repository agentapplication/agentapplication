...
framework decides how the project is configured, built, and released. An [Agent
Application Platform](/concepts/platform) runs it in that native form instead
of translating every system into a lowest-common-denominator schema.
...
This is a logical map, not a folder convention. A framework may express these
capabilities in code, Markdown, configuration, platform settings, or a mix of
all four. Existing standards fit at specific boundaries: MCP connects tools and
data, Agent Skills carries reusable instructions, Agent Plugins bundles skills
and MCP configuration, AG-UI connects agent backends to frontends, and A2A
connects independent agents. [Section 9 of the paper](/paper#9-the-agent-project-and-capability-model)
maps the capabilities and compares current frameworks.
