...
sidebarTitle: "Review"
description: "How to send holes, counterexamples, boundary cases, and prior art for Working Draft 0.9.1."
---

This is **Working Draft 0.9.1**, and the work is now being developed in public at
...
## Review the category boundary

The paper identifies four defining properties: a persistent agent instance,
agent-directed control flow, a durable computational workspace, and durable
work. If a real product crosses that boundary in a way the definition misses,
we want to understand the case.
...
The paper also assigns responsibilities to frameworks, projects, releases,
instances, platforms, and stores. Useful architecture feedback can identify a
missing responsibility, show that one has been assigned to the wrong element,
or describe a real system whose boundaries do not fit the model.
...
The same repository tracks possible companion contracts. A contract becomes a
candidate for a specification when independent implementations need to exchange
information across a stable boundary. See [Companion
documents](/companion-documents) for the current list.
