...
description: "Working Draft 0.9, August 2026. A vendor-neutral reference architecture for building and operating AI agent systems."
---

**Working Draft 0.9 · August 2026** · [Plain text](https://agentapplication.io/llms-full.txt) · [Cite](/#cite)
...
The architecture applies to products built around persistent,
tool-using agents. A request, event, or schedule can start work. The agent
chooses intermediate actions within application guardrails, while its instance
retains a workspace, authority, artifacts, and unfinished work after the run
ends. This paper calls the complete product an **Agent Application**.
...
agent loop. This paper defines an **Agent Application Framework** as a harness
that includes a coherent project structure, development tools, evaluations,
packaging, deployment, and production runtime support. An **Agent Application
Platform** turns a tested project into an immutable release, provisions and
operates long-lived instances with durable workspaces, and supplies identity
management, security controls, governance, observability, evaluation, and fleet
management.

Long-lived instances create problems that do not appear in a single model call.
Their workspaces accumulate different facts, artifacts, instructions, generated
code, and unfinished work. Instances can diverge in program as well as state.
Operators must preserve that work, enforce authority outside the model, test
new releases against existing instance state, and keep enough evidence to
explain or recover consequential actions.

The paper provides a layered system model, a capability model, lifecycle and
security views, a method for applying the architecture, and a worked example.
It introduces **Agent Application Programming**, the discipline of decomposing
a use case across code, natural-language programs, tools, agents, workflows,
state, and policy. It also defines the **Agent Application Store** role and locates
current standards such as the Model Context Protocol (MCP), Agent Skills, Agent
Plugins, and Agent2Agent (A2A) at the boundaries they cover.
...
|---|---|
| Application architects and developers | Define the application's job and privacy boundary, decompose its program, assign capabilities, and design its release and instance lifecycles. |
...
|---|---|---|
| Understand the category | What is an Agent Application, and how does it differ from familiar software? | [Definition](#1-what-is-an-agent-application), [web and mobile comparison](#2-from-web-and-mobile-applications-to-agent-applications), [category boundary](#7-recognizing-an-agent-application) |
| Design an application | How should a team divide behavior and turn a use case into a tested project? | [Design principles](#4-architectural-drivers-and-design-principles), [build-and-operate method](#9-a-build-and-operate-method), [programming model](#11-a-hybrid-programming-model), [capability model](#12-the-agent-project-and-capability-model) |
...
| Platform operations and governance | Where are authority and trust enforced, and how is a fleet operated? | [Agent Application Platform](#18-the-agent-application-platform), [security, reliability, and governance](#20-security-reliability-and-governance) |
| Work on ecosystem boundaries | Which boundaries have protocols, and where might shared contracts or services help? | [Standards and portability](#19-standards-contracts-and-portability), [Agent Application Stores](#21-agent-application-stores), [open questions](#22-open-questions) |
...
| Stage | Web application | Agent Application |
|---|---|---|
| Select a framework | The developer chooses a web application framework that defines project conventions, supported languages and runtimes, and base capabilities. | The developer chooses a harness / Agent Application Framework that defines the agent loop, native project conventions, model and tool integration, and base capabilities. |
| Program the application | The developer writes routes, user interfaces, business logic, and data access within the framework's conventions. | The developer composes conventional code, natural-language programs, declarations, knowledge, tools, agents, workflows, and policy in an Agent Project. |
| Test and release | Tests exercise the application before the build system creates a deployment artifact. | Evaluations exercise behavior across the selected model, framework, tools, and workspace fixtures before the platform creates an immutable release. |
| Deploy and connect services | A web application platform deploys the artifact and connects it to databases, caches, queues, secrets, and outside services. | An Agent Application Platform deploys the release and connects its instances to models, tools, durable workspaces, identity, authority, schedules, and artifact services. |
| Operate at scale | The platform provides tenant isolation, request routing, scaling, fault tolerance, logging, security controls, and staged deployments. | The platform provides instance isolation, session routing, suspension and resume, scaling, security, governance, observability, and semantic upgrades across the fleet. |
| Use the application | Browser and API requests reach application code and account or tenant data. | Widgets, channels, APIs, and events return to the same persistent instance and the durable work it has accumulated. |
...
*Figure 2. Seven-layer Agent Application stack.*

```mermaid
%%{init: {"flowchart": {"rankSpacing": 12, "nodeSpacing": 10}, "themeVariables": {"fontSize": "14px"}}}%%
flowchart LR
    subgraph Stack["Seven-layer stack"]
        direction LR
        L1["L1 Model"] --> L2["L2 Agent compute"] --> L3["L3 Agent harness"]
        L3 --> L4["L4 Agent Application Project"] --> L5["L5 Agent Cloud"]
        L5 --> L6["L6 Agent productization"] --> L7["L7 Agent use cases"]
    end
```

| Layer | Responsibility |
|---|---|
| L7 Agent use cases | The Agent Applications delivered to users and organizations, such as a financial advisor, support agent, coding agent, tutor, or backend operator. |
| L6 Agent productization | Turns running agents into products through user and machine interfaces, tenant access, schedules, artifact delivery, fleet operations, metering, and usage controls. An Agent Application Store may supply acquisition, entitlements, billing, and publisher payments at this boundary. |
| L5 Agent Cloud | Runs long-lived instances with isolated durable workspaces, resumable execution, identity and authority controls, checkpoints, observability, and per-instance metering. |
| L4 Agent Application Project | Holds the framework-native program used to build releases, including code, natural-language programs, declarations, knowledge, dependencies, tools, workflows, policy, and evaluations. |
| L3 Agent harness | Assembles context, invokes models and tools, loads skills and subagents, and runs the agent loop. |
| L2 Agent compute | Provides the isolated conventional compute used for code, tools, shell commands, browsers, and filesystem operations. |
| L1 Model | Interprets natural-language programs, reasons over current state, generates content, and chooses actions within application guardrails. |
...
from different systems. An Agent Application Framework joins an L3 harness to
a coherent L4 Agent Application Project and development lifecycle, then
supports deployment and production runtime. An Agent Application Platform
normally spans L5 and L6 and may also supply development tools for L4 projects.

[Karta](https://karta.sh/platform) is one implementation that provides
development tools at L4 and platform services across L5 and L6.
...
*Figure 3. Logical view of an Agent Application.*

```mermaid
flowchart TB
    Developer(["Developer"]) --> Framework["Harness / Agent Application framework"]
    Framework --> Project["Agent Project"]
    Project --> Release["Immutable release"]
    Release --> Platform
    Platform["Agent Application Platform"] --> Instance["Agent instance"]
    Surface["Widget, channel, API, or event"] --> Instance
    Instance --> Runtime["Framework runtime"]
    Instance --> Workspace["Durable workspace"]
    Runtime <--> Workspace
    Runtime --> Model["Model and compute"]
    Runtime --> Services["Tools and external services"]
    Workspace --> Artifacts["Artifacts and durable results"]
```
...
## 4. Architectural drivers and design principles

Seven design principles explain why the architecture separates its elements.

1. The complete application is the unit of development and evaluation. Its
   conventional code, natural-language programs, declarations, knowledge, and
   dependencies change behavior together.
2. The privacy boundary determines the instance boundary. Each privacy domain
   receives a stable instance and durable workspace even when its compute is
   suspended.
3. A shared release and an instance's local state have different owners and
   lifecycles. Updating one should not silently replace the other.
4. Long-lived instances require semantic upgrades. Evaluation covers clean test
   fixtures and the workspaces the release will affect.
5. Identity, permissions, budgets, network access, and approval rules are
   enforced outside program material that the agent can change.
6. Consequential work needs evidence and recovery points. An operator should be
   able to explain an action, resume interrupted work, and respond when an
   outside effect cannot be reversed.
7. Useful work should survive a change of provider. Export and migration cover
   artifacts, workspace state, lineage, and authority references.
...
## 6. Where people encounter AI agents
...
Section 7 defines the category through its execution model.

---

## 7. Recognizing an Agent Application
...
### 7.1 How to decide who gets an agent instance
...
### 7.2 A durable computational workspace
...
an external system of record cannot represent. Section 16 draws that boundary.
...
### 7.4 What is not an Agent Application?
...
| **Agent Application Programming / Agent App Programming** | The discipline of decomposing a use case across code, natural-language programs, tools, agents, workflows, state, and policy. |
...
The architecture is useful when it turns an idea into explicit design
...
*Figure 4. Release and instance lifecycle.*

```mermaid
flowchart TB
    Project["Agent Project"] --> Evaluate["Build and evaluate"]
    Evaluate --> Release1["Release 1.0"]
    Release1 --> InstanceA["Maya's instance"]
    Release1 --> InstanceB["Another instance"]
    InstanceA --> Session1["Session"]
    Session1 --> WorkspaceA["Durable workspace"]
    InstanceA --> Session2["Later session or schedule"]
    Session2 --> WorkspaceA
    WorkspaceA --> LocalSkill["Maya's modified version 1 skill"]

    Project2["Revised Agent Project"] --> Evaluate2["Build and evaluate"]
    Evaluate2 --> Release2["Release 1.1"]
    Release2 --> Upgrade["Staged semantic upgrade"]
    LocalSkill --> Upgrade
    Upgrade --> Continued["Version 1.1 with Maya's change preserved"]

    Continued -..-> WorkspaceA
```
...
designates it as an instruction source. Section 20.4 develops these controls.
...
Section 15.5 explains why text diffs alone cannot settle upgrades.
...
attached. Section 20.4 develops the security consequences.
...
One goal of this project is to make these differences visible and give the
community a shared capability model. A harness moves toward becoming a
...
without touching another. Section 7.1 gives examples of where applications draw
that boundary; section 20 covers the controls that enforce it.
...
### 15.5 Upgrades require semantic merges
...
section 6. It presents conversation, session history, artifacts, approvals, and
...
# Part IV: Operating Agent Applications
...
enforce hard limits, as section 20.3 describes.
...
attaches scoped authority, and runs their sessions. A store or publisher may
...
## 19. Standards, contracts, and portability
...
## 20. Security, reliability, and governance
...
*Figure 5. Trust and authority boundaries.*

```mermaid
flowchart TB
    Principal["User, organization, or event source"] --> Identity["Identity and instance routing"]
    Identity --> Runtime["Agent runtime"]
    Untrusted["Documents, messages, web pages, and tool results"] --> Labels["Source and trust labels"]
    Labels --> Runtime
    Runtime --> Policy["Policy and approval boundary"]
    Policy --> Tool["Tool or MCP server"]
    Tool --> Resource["External resource"]
    Runtime --> Sandbox["Sandboxed code execution"]
    Runtime <--> Workspace["Isolated durable workspace"]
    Policy --> Evidence["Audit, checkpoints, and recovery record"]
    Tool --> Evidence
```
...
### 20.1 The agent cannot choose its own authority
...
### 20.2 Review the complete plugin
...
# Part V: Open questions and conclusion
...
instance. Release evaluation, staged semantic upgrades, lineage, checkpoints,
and targeted repair address that condition.
...
each framework. The common map shows what the system must account for, who owns
each responsibility, and where
...
Section 20.1 places commerce and payments at the delegated-authority boundary;
...
**Named systems and formats.** Claims about named systems in sections 1, 3, 6,
7, 9, 11, 12, 18, 19, and 20, as well as Appendix D, come from each vendor's own
