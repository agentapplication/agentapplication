...
description: "Working Draft 0.7, August 2026. The full paper defining Agent Applications, their lifecycle, and the Agent Application Platform."
---

**Working Draft 0.7 · August 2026 · Amol Kelkar** · [Plain text](https://agentapplication.io/llms-full.txt)
...
Standards already cover a few components of Agent Applications. MCP connects agents to tools, Agent Skills package reusable instructions, A2A supports communication among agents and agentic applications, and emerging protocols govern commerce and payments. Important gaps remain around artifacts, identity and authority, workspace portability, releases, updates, and the application-instance lifecycle.
...
| Agent project and release | A native project directory contains the developer-authored program, configuration, knowledge setup, and evaluations. Publishing fixes that source and its tested runtime requirements in an immutable release. |
...
Figure 1 follows one application from development to use.
...
| Natural language programs | [Agent Skills](https://agentskills.io/specification) and related formats package procedural instructions so a harness can load ane execute them. |
...
surface. Appendix B covers earlier uses of the name.
...
An Agent Application combines conventional code, natural-language programs,
and knowledge. Each behaves differently and needs a different kind of test.
...
designates it as an instruction source. Section 19.3 develops these controls.

These instructions can specify more than preferences or background facts. An
agent developer can write a multi-step process: inspect the source material,
draft a change, ask a review subagent to check it, run the required tests, and
report the result. The developer may also specify a dynamic workflow graph
that the agent instantiates to execute a complex process:
a deep research request may use multiple subagents to plan different aspects of a
decomposed query, second level subagents may research each aspect followed by
a critic subagent and finally a set of subagents that bring together all findings into a
report. When executing such dynamically generated process graph, the harness
loads the relevant instructions, and the LLM
interprets them against the current state, calls tools, and steers
the process through contextual course corrections and elaborations.

Frontier models have become better at following detailed instructions and
chaining tool calls, making longer procedures practical as natural-language
programs. Direct interpretation is the lightest-weight form. When the work needs
named steps, compiler checks, and inspectable execution, a system can add more
structure. [Playbooks](https://www.runplaybooks.ai/#video-intro), a Natural Language programming based harness, developes this by
compiling structured Markdown programs into an intermediate language to disambiguate intent
and providing step debugging through instructions with breakpoints, variables, and a call stack,
thus elevating Natural Language into a full programming language with verifyable execution traces.
The [Agent Skills specification](https://agentskills.io/specification) does not provide execution guarantees, instead relies on
[evaluations](https://agentskills.io/skill-creation/evaluating-skills) for reliable execution.
...
attached. Section 19.3 develops the security consequences.

### 8.4 Two kinds of execution
...
### 8.5 Test the whole application

Each part of an Agent Application needs a different test. Developers test code with ordinary software
tests. They test natural-language instructions by giving the agent representative
jobs and judging its behavior. They check knowledge for source, freshness, and
permission.
...
code, natural-language programs, knowledge setup, configuration, and evaluations
in the native layout of the selected system.
...
Portability requires a smaller shared contract. Alongside its native files, the
project declares the tools, permissions, workspace features, tests, and upgrade
rules it needs. Two platforms can satisfy those requirements while running the
same native project, although different models or framework versions may still
...
manifest. Section 18 describes which parts need shared contracts.
...
**The release** is one immutable version of the agent project, fixing source,
declared runtime requirements, evaluations, and migration rules. Publishers
change the application by cutting a release. Prior releases stay available for
...
many, started by a user, an event, or a schedule. A session may or may not
have a formal "finished" state. A given session uses application code in the
agent instance at time time it is invoked.
```text
Agent Application
  |-- Release 1
  |-- Release 2
...
how it behaves. Installing a tool changes what it can do. Granting a credential
changes what it may reach. Each step needs stronger review.
...
instances is a different and open problem, noted in section 23.
...
them. Code and policy systems must enforce hard limits, as section 19.2 requires.
...
| Identity and authority | Gives each instance scoped credentials, approval rules, limits, and revocation |
...
tools and external data. Agent Skills packages reusable instructions. A2A
supports collaboration between agents and agentic applications. Commerce and
payment protocols carry purchase intent, authorization, and settlement. Appendix
D.4 maps these protocols in detail.

The missing layer is a set of application-level contracts for Agent
Applications, Agent Artifacts, identity and delegated authority, monetization,
communication, releases, instances, and workspaces. These contracts can compose
with existing protocols instead of replacing them.
...
The developer asks for permissions. The customer and platform decide what to
grant. The agent can use that authority but cannot expand it. Permissions can
expire or be revoked while the instance is suspended or running.
...
intent, checkout state,
authorization, payment requirements, and receipts. Policy code still enforces
those limits and handles revocation. Appendix D.4 lists the current protocols.

### 19.2 Put hard limits in code
...
A permission such as `crm.write` is too broad by itself. The platform must also
check which customer account the agent is changing. A persuaded agent then
cannot reach another account or send private files to an unapproved server.
...
### 19.3 Remember where information came from
...
### 19.5 Keep enough evidence to explain and recover
...
A release now includes code, natural-language instructions, knowledge setup, and
behavior tests. Developers must review instruction changes with the same care as
code changes because both can alter what the application does. Shared formats
...
and evaluations. Secrets and user data stay outside the source folder.
...
### 24.5 Turn the use case into evaluations
...
### 24.6 Deploy an immutable release
...
The platform validates and packages the native folder, records the resolved
harness and runtime dependencies, and creates an immutable release. It keeps
secrets outside that release. The developer provisions a staging instance using
the application's real privacy-boundary rule, attaches test credentials, and
runs smoke tests through the hosted agent widget. The test covers a fresh
instance, resume from a populated workspace, tool approvals, schedules, and
artifact rendering.

### 24.7 Deliver the application to users
...
### 24.8 Operate and upgrade the fleet

A new version returns to the same local loop: edit the native project, run the
evaluations, publish a candidate, and roll it out in stages. The platform
...
### 24.9 Deliver and share artifacts
...
Developers build these products as hybrid programs of code, natural-language
instructions, and knowledge. Agent Application frameworks provide the project
structure and development tools. Agent Application Platforms turn tested
releases into long-lived instances and operate their workspaces, authority,
interfaces, fleets, costs, and monetization.
...
MCP, Agent Skills, A2A, and commerce protocols cover some interfaces in this
stack. Contracts for applications, workspaces, artifacts, identity, authority,
updates, and monetization would connect the rest. Developers could then sell
...
# Appendix A: A candidate Agent Project manifest

This appendix is a design proposal, offered as a starting point rather than a
specification. In ordinary use, *Agent Application* or *agent* can refer to this
...
Figures and citations that move. Stated as of August 4, 2026. The body's claims are
...
August 4, 2026 and should be rechecked before citation.
...
The current status of these documents is maintained at
...
hooks, and fallback routes against which the release was tested. Compatibility
means that the required features exist. It does not mean that two runtimes will
...
| Behavior | Skills, procedures, planning rules | Approved author after evaluation |
| Capability | Tools, connectors, generated code, packages | Administrator after review and sandbox testing |
| Structure | New agents, schedules, event loops | Administrator |
...
Several current formats cover part of the portability contract:
...
| [AP2](https://ap2-protocol.org/) and [x402](https://docs.cdp.coinbase.com/x402/welcome) | Verifiable payment intent and authorization; programmatic payment requirements and settlement |
...
No current format carries the complete agent project, workspace requirements,
behavior tests, identity and authority record, monetization data, communication
events, artifacts, and instance lineage together. New contracts will form at
the exchange boundaries that existing formats cannot cover.
