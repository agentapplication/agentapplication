...
build, deploy, and manage. As a working paper, it also gives the community a
place to revise the model as agents take on harder work, including running
large businesses and conducting frontier research.
...
ChatGPT, Claude, Lovable, and Lightfield look similar at first because each uses
an agent widget. Their jobs differ. ChatGPT and Claude are general assistants,
Lovable builds software, and Lightfield works with customer data. In every case,
the agent can return to the same body of work across multiple sessions.
*Chatbot*, *copilot*, and *workflow* each describe one aspect of these products,
but none names the whole application.
