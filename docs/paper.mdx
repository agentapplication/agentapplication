...
| Agent project and release | A native project directory contains the developer-authored program, configuration, knowledge setup, dependencies, and evaluations. The chosen framework and platform turn it into a fixed, inspectable version for deployment. |
...
span the range from agent harnesses to emerging application frameworks. They
all run or support the agent loop, but they take responsibility for different
parts of the development and operating lifecycle. A harness becomes a full
Agent Application framework as it adds a coherent approach to project
structure, local development, debugging, evaluation, deployment, and production
operation.
...
framework. This directory is the **agent project**. It contains the code,
instructions, knowledge, configuration, and tests used to build and run the
application.

The project should stay in the framework's native form. Next.js and Rails both
build web applications, but they do not share a route file, project manifest,
or release bundle. Their common ground is the web platform and its protocols.
Agent frameworks should work the same way. They can provide different project
layouts and programming models while supporting the same broad set of
application capabilities.

Choosing a framework is therefore an architectural decision. Its model support,
context strategy, tool system, execution environment, state model, event stream,
and debugging facilities shape the behavior and cost of the application.

### 9.1 A capability model for Agent Applications

Most projects need some subset of the capabilities below. This is a superset,
not a checklist. A small application may use only an agent program, a few
tools, an interface, and some tests. A long-running business application may
need every row.
...
This model gives framework authors a shared design vocabulary. It does not
require a literal folder, configuration key, or API for any row.

### 9.2 A logical project anatomy

The same capabilities can be shown as a logical project structure:
...
These labels are not filenames. One framework may express the whole project in
ordinary code. Another may use Markdown instructions, configuration files, and
conventional directories. A managed platform may keep some operational settings
outside the repository. Appendix A shows one concrete layout, and section 24
shows how a native framework project fits the model.

### 9.3 Where existing standards fit

Shared formats are most useful where independently built systems meet. They are
less useful inside a framework that already controls both sides of an
interface.
...
These standards are closer to HTTP, HTML, and package formats than to a common
Next.js or Rails release. A framework can support them while keeping its own
source layout, build process, and deployment model.

### 9.4 How current frameworks expose the capabilities

The table below is a snapshot of the first-class paths documented by five
frameworks as of August 8, 2026. It is not a benchmark or a conformance table.
An entry such as "application-owned" means the framework leaves that capability
to application code or a separate platform, not that the capability is
impossible to build.
...
The repeated rows show where the category is settling. Every framework provides
an agent program, tools, composition, and events, and each documentation set
addresses state in some form. The differences show where framework design still
matters: instruction packaging, workspace semantics, evaluation, UI
integration, and deployment are not yet one common layer.

### 9.5 Plugins are components, not applications

An Agent Plugin contributes components to a project or host. The application
still owns its primary program, product identity, state model, authority rules,
tests, and operating lifecycle. A plugin-provided reviewer remains part of the
application unless it is deployed as an independently addressable application
with its own identity and authority boundary.
...
Four decisions remain separate when a project uses a plugin:
...
Completing one decision does not complete the next. A skill can change behavior
without adding a tool. An MCP server can add a capability before it receives a
credential. A hook may execute without the model choosing it.

Imported components also need unambiguous names in logs, permissions, tests,
and lineage. A framework can qualify a short name such as `review` with the
plugin identity and component type. The exact syntax belongs to that framework
until separate implementations need to exchange it.
...
**The release** is the fixed version of the native project that a publisher
deploys. Its representation depends on the framework and platform. A production
release should identify the source revision, resolved dependencies, framework,
harness and model versions, active components, requested authority, runtime
requirements, evaluations, and migration rules well enough to inspect and
reproduce what was deployed. Prior releases stay available for inspection and
rollback.
...
Standards already cover several boundaries in the stack. MCP connects an agent
host to tools and external data. Agent Skills packages reusable instructions.
Agent Plugins packages skills and MCP server configurations for compatible
clients. AG-UI connects an agent backend to a frontend. A2A supports work
between independently addressable agents. Commerce and payment protocols carry
purchase intent, authorization, and settlement. Section 9.3 and Appendix D.4
map these protocols in more detail.
...
Portability should instead be pursued at demonstrated exchange boundaries. A
frontend can talk to several agent backends through AG-UI. A tool can serve
several hosts through MCP. An Agent Skill or Agent Plugin can move between
compatible clients. Independently deployed agents can collaborate through A2A.
These are useful contracts because different implementations meet at each
boundary.

Other boundaries may deserve standards later. Artifacts, workspace export,
lineage, delegated authority, and billing records are candidates because users
and operators may need to move them between platforms. Such work should begin
with two independent implementations and a concrete exchange problem, not with
an attempt to normalize every framework's internals.
...
rather than a general ranking. Start with the capability model in section 9.1,
then compare the candidates on the details that matter for the application:
...
Add each plugin through the package or installation mechanism supported by the
...
package lock, an OCI digest, or a framework-specific plugin record can all serve
this purpose. There is no common Agent Application lockfile.
...
The platform builds the native project using the framework's own dependency and
release mechanisms. It records enough information to inspect and reproduce what
was deployed, including source revision, dependency digests, harness and model
versions, active components, tests, and requested authority. Secrets stay
outside the release. The developer provisions a staging instance using the
application's real privacy-boundary rule, attaches test credentials, and runs
smoke tests through the hosted agent widget. The test covers a fresh instance,
resume from a populated workspace, tool approvals, schedules, plugin data, and
artifact rendering.
...
interfaces in this stack. Frameworks can adopt those standards without sharing
a project layout or release format. Future portability work should focus on the
parts that users and operators actually need to exchange, including artifacts,
workspace exports, lineage, identity, authority, and billing records. Developers
could then reuse components across frameworks and move long-lived work between
platforms without flattening each framework into a lowest common denominator.

---

# Appendix A: An illustrative project layout

This appendix shows how one project might expose the capability model from
section 9. It is a design example, not a portable package format. A framework
may use different filenames, keep several capabilities in one module, or store
operational configuration outside the repository.
...
Another framework might place the primary instructions in Markdown, define
agents in YAML, register tools with decorators, or keep evaluations in a hosted
service. The logical capabilities remain recognizable even though the files do
not match.
...
Figures and citations that move. Stated as of August 8, 2026. The body's claims are
...
**Framework capability snapshot.** Section 9.4 summarizes documented,
first-class paths rather than every possible integration. The sources are the
...
documentation establishes the state, tool, composition, event, evaluation, and
deployment entries. The table reports how each framework presents the
capability, not whether a developer could add another implementation.
...
**Named systems and formats.** Claims about named systems in sections 6, 9, 17,
18, and 24, as well as Appendix D, come from each vendor's own documentation as
of August 8, 2026 and should be rechecked before citation.
...
| Programming Agent Applications | The capability model, framework design patterns, harness interpreter, and evaluation |
...
The paper now contains the first version of the Agent Application capability
model. The other documents remain planned. A proposed contract should become a
specification only after independent implementations need to exchange the same
...
together. That absence does not by itself justify one all-encompassing format.
New contracts should form only at exchange boundaries where independent
implementations have demonstrated a need to interoperate.
