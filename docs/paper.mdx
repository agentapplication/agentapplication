...
```mermaid
flowchart TB
    Developer(["Developer"]) --> Project["Agent Application Project"]
    Publisher(["Publisher"]) --> Project
    Project --> Platform["Agent Application\n Platform"]
    Publisher --> Store["Agent Application Store"]
    Publisher -. fleet, cost, and \naccess management .-> Platform
    User(["User or organization"]) --> Store
    Store -. user acquisition\n and entitlement .-> Platform
    Platform --> Instance["Agent instance"]
    User --> Surface["Widget, channel, \nAPI, or event"]
    Surface --> Instance
    Instance --> Model["Model and compute"]
    Instance --> Services["Tools and external services"]
    Instance --> Workspace["Durable workspace"]
    Workspace --> Artifact["Artifact or durable result"]
    Artifact --> Recipient(["Authorized recipient"])
```
...
| Harness / Agent Application framework | A harness assembles context, invokes models and tools, and runs the agent loop. An Agent Application Framework is a harness that also includes coherent project structure, development tools, evaluations, packaging, deployment, and production runtime support. |
...
| Agent Application Platform | Packages immutable releases and operates long-lived instances with durable workspaces, identity management, security controls, governance, observability, evaluation, and fleet management. |
...
*Figure 2. Logical view of an Agent Application.*
...
*Figure 3. Release and instance lifecycle.*
...
*Figure 4. Trust and authority boundaries.*
