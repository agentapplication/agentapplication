...
framework's native files and supplies any validation or build rules.
...
A project manifest declares dependency intent: plugin identity, source,
...
resolved dependency graph. It fixes application source, plugin and conventional
package contents, framework and harness versions, host extensions, component
activation, requested authority, runtime requirements, evaluations, and
migration rules. Publishers change the application by cutting a release. Prior
releases stay available for inspection and rollback.
...
isolation, retention, export, checkpoint, and deletion rules. It does not share
plugin data across privacy domains unless the application has defined a shared
instance. Secrets stay in the managed secret store rather than either root.
...
The release keeps a component inventory analogous to a software bill of
materials. For each plugin it records the source, version, digest, exported
