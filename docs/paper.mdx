...
A project can group its source material like this. The names describe parts of
the project, not required filenames or a portable package format. Each
framework maps them onto its own files. A small project omits unused parts.
A platform may keep some operational settings outside the repository.
...
Natural-language instructions are the primary program material a harness
loads: a root instruction file, skills, and subagent definitions. Computer-code
tools are deterministic functions, hooks, and other in-repo code. System
integrations declare MCP servers and other connectors. Evaluations test the
assembled application. Package dependencies pin libraries and plugins.
Platform configuration is what a platform reads to package a release and
create instances.

The capability model in section 12.1 still names runtime responsibilities
such as workspace, interfaces, triggers, and policy. Those often live in
the parts above, in the framework, or on the platform rather than as extra
top-level folders. Appendix A shows one larger layout. Section 9 applies the
model as part of the build-and-operate method.
...
This example maps section 12.2 onto one possible layout. It is not a portable
package format. A framework may use different filenames, keep several parts in
one module, or store operational configuration outside the repository.
...
|   `-- retirement-analysis/
|       `-- SKILL.md
|-- tools/                         # computer-code-tools
|   |-- financial-data.ts
|   `-- analysis.ts
|-- workflows/                     # computer-code-tools
|   |-- account-review.ts
|   `-- retirement-review.ts
|-- mcp.json                       # system-integrations
|-- evals/                         # evaluations
|   |-- behavior/
|   |-- security/
|   `-- regression/
...
|-- migrations/                    # ...
`-- <platform-native configuration>  # platform-configuration
```

A Goose project might use `AGENTS.md`; a Claude Code project might use
...
short public argument is the [site overview](https://agentapplication.io/).
