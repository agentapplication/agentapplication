...
just as they build web applications with web application frameworks. Today's
agent harnesses are early versions of these frameworks. As they mature, they
will cover application formats, coding conventions, development tools,
evaluations, connections to external systems, audit trails, and alerting.
Developers deploy a tested project to an Agent Application Platform. The
platform packages the project as an immutable release, creates and operates
long-lived instances from that release, and provides fleet management, cost
...
MCP, Agent Skills, Agent Plugins, hooks, agent loops, and common runtime tools
for shell and filesystem access, code generation, and code execution. This paper
steps back from the individual pieces to describe the application that takes
shape when developers assemble them.

The paper documents that application shape and points to framework tooling,
cloud services, and standards that could make Agent Applications easier to
build, deploy, and manage. It also introduces Agent Application Programming:
the discipline of decomposing a use case across code, instructions, tools,
agents, workflows, state, and policy. As a working paper, it gives the
community a place to revise the model as agents take on harder work, including
running large businesses and conducting frontier research.
...
| **Agent Project** | The native source directory for a harness / Agent Application framework that a developer commonly calls an "agent" or "Agent Application." It contains the code, natural-language programs, declarations, policies, dependencies, and tests used to create releases. |
...
- breaking a use case into model judgments and deterministic operations;
- deciding what belongs in the primary agent, reusable instructions, skills,
  tools, hooks, specialist agents, and runtime policy;
- practicing graph engineering: designing dynamic workflows and deciding which
  transitions are fixed by code and which the model chooses at runtime;
- bringing in external capabilities through plugins, MCP servers, libraries,
...
An Agent Application begins as a source directory written for a particular
harness / Agent Application framework. This directory is the **agent project**.
It contains the code, instructions, knowledge, configuration, and tests used to
...
Other common work belongs in tools and services. Frameworks can simplify local
development, evaluation, and deployment. Cloud platforms can provide durable
workspaces, identity, scheduling, fleet operations, and cost controls. A
standard becomes useful when independent implementations need to exchange
...
Install the chosen harness / Agent Application framework on a local development
machine, create a version-controlled folder, and make the smallest useful agent
run. Preserve its native layout. A Claude Code project might begin like this:
...
Figures and citations that move. Stated as of August 9, 2026. The body's claims are
