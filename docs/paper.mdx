...
produce or maintain durable results. A request, event, or schedule can start the
work. The agent evaluates the current state, chooses intermediate steps, and
uses tools to carry them out. Examples include a life coach that supports one
person for years, a virtual marketing employee that works across a team's
campaigns, and a SaaS assistant that guides a customer from onboarding to a
mature program.
...
Developers build these products with a harness / Agent Application framework.
...
Instances accumulate different knowledge, artifacts, instructions, and
...
and unfinished analysis needed to resume. The harness / Agent Application
framework runs the reasoning loop, while tools connect the agent to her
calendar, notes, documents, and messages.
...
The definition does not require a person to approve every action. A life coach
...
The Life Coach Agent therefore gets one instance per user. A human resources
(HR) helper that handles personal employee matters also gets one per employee.
A business-to-business (B2B) support agent can keep many tickets for one
customer in the same workspace, while different customers get different
instances. A virtual marketing employee can have one instance shared by a whole
team because the team itself is the privacy boundary.
...
A chat interface does not necessarily indicate an Agent Application. A copilot
can qualify when a persistent instance carries its work across sessions. The
same interface can front a stateless service that forgets everything when the
conversation ends.
...
is another boundary case: it monitors configured package ecosystems and opens
...
authentication, and authorization become part of the tool boundary. Under the [MCP authorization
...
- designing dynamic workflows, sometimes called graph engineering, and deciding
  which transitions code fixes and which the model chooses at runtime;
...
The project stays in the framework's native form. Next.js and Rails both build
...
The choice of framework affects the finished application. Its model support,
...
The table collects capabilities found across current Agent Applications. Each
project uses the ones its job requires. A small application may need only an
...
|---|---|---|
| Agent program | A primary agent, model configuration, instructions, context assembly, tool loop, stop conditions, and structured outputs | Framework |
...
and what remains in application or platform code. The capability names do not
specify folders, configuration keys, or APIs.
...
|---|---|---|
| [MCP](https://modelcontextprotocol.io/specification/2025-11-25/basic) | An agent host connecting to tools, resources, and prompts | The agent loop, project layout, workspace, or application release |
...
These standards play roles closer to HTTP, HTML, and package formats than to a
shared Next.js or Rails release. A framework can support them while keeping its
own source layout, build process, and deployment model.

### 9.4 How current frameworks expose the capabilities

The table records how five frameworks documented these capabilities on August
...
### 9.5 Plugins are components, not applications

An Agent Plugin adds components to a project or host. The application still
owns its primary program, product identity, state model, authority rules, tests,
and operating lifecycle. A reviewer supplied by a plugin is part of the
application unless someone deploys it as a separate application with its own
...
Frameworks and package managers can add those features without changing the
plugin format.

Using a plugin involves four separate decisions:

1. Resolution selects a package and source.
2. Installation materializes and verifies its content.
3. Activation exposes selected components to the agent.
4. Authorization grants access to capabilities, accounts, or credentials.

These decisions are independent. A skill can change behavior without adding a
tool. An MCP server can add a capability before it receives a credential. A
hook may execute without the model choosing it.

Logs, permissions, tests, and lineage need an unambiguous name for every
imported component. A framework can qualify a short name such as `review` with
the plugin identity and component type. Each framework can choose its own syntax
until separate implementations need to exchange these names.

The [Hello World Agent Application](https://agentapplication.io/examples/hello-world)
shows a minimal project with instructions, one file-writing tool, a persistent
workspace, and a durable artifact.

---

## 10. Application, release, instance, and session

A publisher turns the source project into a fixed version for deployment: a
release. Deployment creates instances from that release, and each period of
activity becomes a session. These four lifetimes govern the running
application.

**The application** is the product identity: what a company offers and customers
use. It continues across releases.

**The release** is a fixed version of the native project that a publisher
deploys. Each framework and platform has its own release format. A production
release should preserve enough information for an operator to inspect and
reproduce the deployment. This includes the source revision, resolved
dependencies, framework, harness and model versions, active components,
requested authority, runtime requirements, evaluations, and migration rules.
Prior releases stay available for inspection and rollback.
...
release by updating its project files. If the instance has modified those files,
the upgrade may require a semantic merge.
...
**Current working state** is updated in place. Its size tends to reflect the
application's current work rather than its full history. Depending on the
...
**Historical state** accumulates as the instance works:
...
Historical state determines much of a long-lived instance's carrying cost. A
...
The Agent Plugins specification defines two storage roots. For a plugin packaged
with an application, `PLUGIN_ROOT` is immutable release content. `PLUGIN_DATA` is
...
managed secret store rather than either root. Plugin upgrades that change
persistent data include migrations in the release plan.
...
place two entities in one workspace and assume that folder names or file
permissions will keep their information apart.
...
do, and what it has done. The workspace contains the durable working state and
computational environment that the agent uses. Keeping them separate lets an
operator access, move, or restore the workspace without changing the agent's
identity or permissions. It also lets an operator keep per-instance
configuration outside the workspace and out of the runtime agent's reach.

### 11.4 Memory and workspace

A memory system selects facts or past interactions for the model's context.
Whether it stores that information in files, a database, or another service,
persisted memory belongs to the logical workspace when the instance can retrieve
it. Applications can choose memory structures and retrieval methods suited to
their work.
...
activation in lineage, evaluates it in isolation, and keeps it out of the shared
release. A plugin derived from untrusted content cannot promote itself into a
publisher release.
...
When instances may diverge from the release, upgrading a fleet becomes a
semantic update. The platform applies the publisher's intended behavior to each
instance while preserving unrelated local changes. It then tests the result
against that instance's workspace. Any issue found during the upgrade blocks
that instance's upgrade and creates a review task for the fleet manager.
...
An **Agent Artifact** is a portable, versioned, access-controlled work product. The
...
checkpoints, and file count. This is why retention policies and workspace size
caps matter.
...
The company adds AI to its existing product. The copilot is the delivery form;
the system behind it can still be an Agent Application. The existing product
remains the primary work surface and may remain the system of record. Microsoft
365 Copilot inside Word and Excel, and Canva AI inside its design editor, are
current examples of the embedded form.
...
directly. In exchange for taking on the operating burden, it controls the
application's behavior, instance lifecycle, workspaces, compliance, cost, and
product surfaces.
...
An Agent Application Platform can provide these operating services across many
applications: deployment, instance and fleet management, security controls,
cost tracking, and monetization.
...
An **Agent Application Platform** turns a native project for a harness / Agent
Application framework into an operating product. It packages an immutable
release, provisions instances at the application's privacy boundary, attaches a
durable workspace and scoped authority to each instance, and runs the sessions
that perform the work. It also supplies the interfaces, fleet controls, cost
controls, and commercial services needed to distribute the application.
...
Existing standards already cover specific boundaries in the stack: tools,
reusable instructions, component packages, agent events, communication between
independent agents, and commerce. Section 9.3 and Appendix D.4 map these
protocols in more detail.

The rest of the stack does not automatically need a standard. Frameworks can
simplify local development, evaluation, and deployment. Cloud platforms can
provide durable workspaces, identity, scheduling, fleet operations, and cost
controls. A standard becomes useful when independent implementations need to
exchange information or commands.

This paper does not propose a common Agent Project or release format. Current
frameworks do not share enough structure to justify one. A project written for
Claude Agent SDK, Google ADK, LangGraph, OpenAI Agents SDK, or Vercel AI SDK
should keep the source layout, build process, and release mechanism of that
framework.

The practical place to standardize is a boundary where independent systems
exchange information. A frontend can talk to several agent backends through
AG-UI. A tool can serve several hosts through MCP. An Agent Skill or Agent
Plugin can move between compatible clients. Independently deployed agents can
collaborate through A2A. Each contract covers a real connection between separate
implementations.

More contracts may follow for artifacts, workspace exports, lineage, delegated
authority, or billing records. Users and operators may need to move this
information between platforms. Work on any of these contracts should start with
two independent implementations and a specific exchange problem. It should not
try to normalize a framework's internal design.
...
document, support ticket, tool result, or old workspace file. Six rules limit
the damage.
...
Appendix B records one current legal retention example.
...
A company builds the **Life Coach Agent** introduced in section 1. Its agent
project contains:
...
The company later publishes `1.1` with a better goal-planning skill. It passes
every release evaluation. After a staged rollout, new sessions use it while
existing workspaces stay in place.
...
goal as current. If the record were inconclusive, they could restore a copy of
the earlier workspace checkpoint in an isolated environment and investigate it
there.
...
rollback would remove a useful improvement from the other users.
...
The policy files explain approval and external-action rules to developers and
